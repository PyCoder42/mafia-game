// Mafia realtime relay — Val Town HTTP val (Deno).
//
// A faithful port of scripts/realtime_server.py: a host-authoritative WebSocket
// relay for multi-device Mafia rooms. Deploy this as a Val Town HTTP val; its
// wss:// URL goes into scripts/config.js as productionRelayUrl.
//
//   - GET (no Upgrade header) -> 200 health check (also what /health returns).
//   - WebSocket upgrade -> join_room / state_update / action_request /
//     request_state / kick_device, with presence + host migration.
//
// State is held in-memory per isolate. For a party game (one room, a few
// players) Val Town serves all of a val's connections from one warm isolate,
// which matches the single-process Python relay. No external storage needed.
...
export default function (req: Request): Response {
  // Plain HTTP (health check / browser visit) — no WebSocket upgrade requested.
